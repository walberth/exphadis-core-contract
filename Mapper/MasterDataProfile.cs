...
public class MasterDataProfile : Profile
{
    public MasterDataProfile()
    {
        ...
        this.CreateMap<Announcement, AnnouncementDto>()
            ...
            .ForMember(dest => dest.MediaId, opt => opt.MapFrom(src => src.MediaId ?? 0))
            .ForMember(dest => dest.DeliveryChannel, opt => opt.Ignore())
            .ForMember(dest => dest.WhatsappPayload, opt => opt.Ignore())
            .ForMember(dest => dest.WhatsappDispatch, opt => opt.Ignore())
        ...
        this.CreateMap<AnnouncementDto, Announcement>()
            ...
            .ForMember(dest => dest.Body, opt => opt.MapFrom(src => src.Body))
            .ForMember(dest => dest.DateToPublish, opt => opt.MapFrom(src => src.DateToPublish))
            .ForMember(dest => dest.MediaId, opt => opt.MapFrom(src => src.MediaId == 0 ? (int?)null : src.MediaId))
            .ForMember(dest => dest.DeliveryChannel, opt => opt.Ignore())
            .ForMember(dest => dest.WhatsappPayloadJson, opt => opt.Ignore())
            .ForMember(dest => dest.WhatsappDispatchBatchId, opt => opt.Ignore())
            .ForMember(dest => dest.RoleIds, opt => opt.MapFrom(src => $"{(src.RoleIds ?? Array.Empty<int>()).ToList().ConvertToIntListString()}"))
            .ForMember(dest => dest.LevelId, opt => opt.MapFrom(src => $"{(src.LevelId != 0 ? src.LevelId.ConvertObjectToString() : string.Empty)}"))
            .ForMember(dest => dest.GradeIds, opt => opt.MapFrom(src => $"{(src.GradeIds ?? Array.Empty<int>()).ToList().ConvertToIntListString()}"))
            .ForMember(dest => dest.Media, opt => opt.Ignore());
